...
class VaccineCenter with ChangeNotifier {
  var centers = [];
  static const states = [
  ...
  Future getCityId() async {
    ...
    final statedoc = states.singleWhere((stat) => stat['state_name'] == state,
        orElse: () => null);

    if (statedoc == null) {
      return null;
    }
    stateId = statedoc['state_id'];
    var cityUrl =
        'https://cdn-api.co-vin.in/api/v2/admin/location/districts/$stateId';
    try {
      final cities_response = await http.get(Uri.parse(cityUrl));
      final cities = json.decode(cities_response.body);
      final citydoc = cities['districts'].singleWhere(
          (cit) => cit['district_name'] == city,
          orElse: () => null);
      if (citydoc == null) {
        return null;
      }
      return citydoc['district_id'];
    } catch (err) {
      print(err);
    }
  }
  ...
  Future<void> getCenters() async {
    ...
    var url =
        'https://cdn-api.co-vin.in/api/v2/appointment/sessions/public/findByDistrict?district_id=$cityId&date=$formattedDate';
    try {
      final response = await http.get(Uri.parse(url));
      centers = json.decode(response.body)['sessions'];
      notifyListeners();
    } catch (error) {
      print(error);
    }
  ...
  List<dynamic> get districtCenters {
    return [...centers];
  }
}
